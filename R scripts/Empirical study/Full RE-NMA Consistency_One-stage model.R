...
for(i in 1:L){
  ...
  jagsfit.NMA[[i]] <- jags(data = data.jag[[i]], parameters.to.save = param.jags, model.file = "./Model scripts/Full RE-NMA One-stage IMOR Pattern-mixture model.txt",
...
write.table(round(LOR, 4), file = "./Output/Empirical study/LOR_One-stage_Results.txt", sep = "\t", quote = F) # LOR.ams[-134, ]
write.table(round(theta, 4), file = "./Output/Empirical study/theta_One-stage_Results.txt", sep = "\t", quote = F) # theta[-430, ]
write.table(round(tausq, 4), file = "./Output/Empirical study/tausq_One-stage_Results.txt", sep = "\t", quote = F)
