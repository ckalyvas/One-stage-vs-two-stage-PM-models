...
for(i in 1:L){
  ...
  jagsfit.NMA[[i]] <- jags(data = data.jag[[i]], parameters.to.save = param.jags, model.file = "./Model scripts/Full RE-NMA Two-stage IMOR Pattern-mixture model.txt",
...
write.table(round(LOR, 4), file = "./Output/Empirical study/LOR_Two-stage_Results.txt", sep = "\t", quote = F) # LOR.ams[-134, ]
write.table(round(theta, 4), file = "./Output/Empirical study/theta_Two-stage_Results.txt", sep = "\t", quote = F) # LOR.ams[-134, ]
write.table(round(tausq, 4), file = "./Output/Empirical study/tausq_Two-stage_Results.txt", sep = "\t", quote = F)
