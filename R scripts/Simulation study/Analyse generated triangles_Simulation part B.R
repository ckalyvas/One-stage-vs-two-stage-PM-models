...
source("./R scripts/Simulation study/Two-stage PM model_Function for First-stage.R") 
       
# Load the 1,000 generated triangle networks with missing outcome data:

load("./R scripts/Simulation study/1000 simulated two-arm networks.RData")
  ...
  for(i in 1:length(compile[, 1])){  # LOOP For SCENARIOS
    ...
    for(l in 1:sim){    # LOOP FOR SIMULATED DATASET
      ...
      jagsfit.one.pm <- jags(data = Data.one, parameters.to.save = c("tausq", "LOR", "SUCRA"), n.chains = n.chains, 
                             ...
                             model.file = "./Model scripts/Full RE-NMA One-stage IMOR Pattern-mixture model.txt", DIC = F, working.directory = getwd())
      jagsfit.two.pm <- jags(data = Data.two, parameters.to.save = c("tausq", "LOR", "SUCRA"), n.chains = n.chains, 
                             n.iter = n.iter, n.burnin = n.burnin, n.thin = n.thin, 
                             model.file = "./Model scripts/Full RE-NMA Two-stage IMOR Pattern-mixture model_Two-arm trials.txt", DIC = F, working.directory = getwd())
